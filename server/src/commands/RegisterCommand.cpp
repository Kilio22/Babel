...
void Babel::Server::Commands::RegisterCommand::handle(const unsigned char *data, const std::size_t, IUser *user) const
{
    ...
    RegisterResponse registerResponse = { Header(COMMAND_TYPE::REGISTER), RESPONSE_CODE::OK };
    const std::string username = registerRequest->username;
    if (username.length() < 3) {
        registerResponse.responseCode = RESPONSE_CODE::WRONG_USERNAME_LENGTH;
    ...
    if (password.length() < 3) {
        registerResponse.responseCode = RESPONSE_CODE::WRONG_PASSWORD_LENGTH;
        return user->getTcpClient()->write(reinterpret_cast<const unsigned char *>(&registerResponse), sizeof(RegisterResponse));
    }
    ...
    try {
        ...
        registerResponse.responseCode = RESPONSE_CODE::USERNAME_TAKEN;
    } catch (const std::exception &) {
        registerResponse.responseCode = RESPONSE_CODE::OTHER;
    }
    return user->getTcpClient()->write(reinterpret_cast<const unsigned char *>(&registerResponse), sizeof(RegisterResponse));
}
